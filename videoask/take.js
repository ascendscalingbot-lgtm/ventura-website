...
export async function bootTake(id) {
  document.body.dataset.mode = "take";
  ...
  state.flow = await hydrateAsk(raw);
  chromeTitle.textContent = raw.title || "ventura.ask";
  document.title = `${raw.title || "Ventura Ask"}`;
  await applyMedia(step());
  render();
}
...
function showMissing(id) {
  ...
  sheet.innerHTML = `
    <div class="sheet-body done-copy">
      <h1>Ask not on this device</h1>
      <p>Recordings live in the browser that created them. Open this link on that phone or laptop, or create a new survey.</p>
      <a class="cta" href="./" style="display:grid;place-items:center;margin-top:28px;text-decoration:none">Create an ask</a>
    </div>
  `;
  announce(`Ask ${id} was not found on this device.`);
}
...
function renderBottom(s) {
  if (s.type === "intro") {
    bottom.innerHTML = `<button class="cta" id="startBtn" type="button">${s.cta || "Start"}</button>`;
    $("startBtn").onclick = startAsk;
    return;
  }
  if (s.type === "choice") {
    bottom.innerHTML = `
      <div class="choices" role="listbox" aria-label="${s.prompt || s.title}">
        ${(s.options || [])
          .map(
            (o) => `
  ...
  if (s.type === "yesno") {
    bottom.innerHTML = `
      <p class="prompt">${s.prompt || s.title}</p>
      <div class="yesno">
...
function renderPermissions(s) {
  ...
  sheet.innerHTML = `
    ...
    <div class="sheet-body">
      <p class="kicker">${s.kicker || "Let’s get started by giving Ventura…"}</p>
      <h1>${s.title || "Access to your mic and cam"}</h1>
...
async function renderComplete(s) {
  hud.hidden = true;
  sheet.hidden = false;
  await saveReply({
    askId: state.flow.id,
  ...
  sheet.innerHTML = `
    ...
    <div class="sheet-body done-copy">
      <h1>${s.title || "You’re in"}</h1>
      <p>${s.body || "We got your answers."}</p>
      ${count ? `<p>${count} answers saved.</p>` : ""}
      <button class="cta" id="again" type="button" style="margin-top:28px">Submit another</button>
    </div>
  `;
  $("doneClose").onclick = () => {
    location.href = "./";
...
function renderText(s) {
  ...
  bottom.innerHTML = `
    <div class="text-box">
      <p class="prompt">${s.prompt || s.title}</p>
      <label class="sr-only" for="answerText">${s.prompt || s.title}</label>
...
async function renderRecorder(s) {
  ...
  state.rec.onstop = async () => {
    ...
    state.draft = { ...state.draft, blob, url: URL.createObjectURL(blob), mediaKey: key };
...
function renderReview(s) {
  ...
  $("useTake").onclick = () => {
    state.answers[s.id] = { type: kind, mediaKey: state.draft.mediaKey };
    if (state.draft?.url) URL.revokeObjectURL(state.draft.url);
    next();
  };
}
...
function syncTime() {
  timeEl.textContent = `${fmt(host.currentTime)} / ${fmt(host.duration || 0)}`;
}
...
$("fsBtn").onclick = async () => {
  const rootEl = document.querySelector(".phone") || document.documentElement;
  if (!document.fullscreenElement) await rootEl.requestFullscreen?.();
...
$("infoBtn").onclick = () => {
  toast(state.flow?.title || "Ventura Ask");
};
...
document.addEventListener("keydown", (e) => {
  ...
  if (s.type === "choice" && ["1", "2", "3", "a", "b", "c"].includes(e.key.toLowerCase())) {
    ...
    bottom.querySelector(`[data-id="${id}"]`)?.click();
  }
});
